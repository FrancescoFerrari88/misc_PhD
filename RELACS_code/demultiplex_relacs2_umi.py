...
def parseArgs(args=None):
    ...
    parser.add_argument('sampleTable', help="""A tab-separated table with four columns: Illumina sample	barcode	sample_name UMI_length

An example is:

Sample_Lib_C838_11	ACTACT	Sample1	2
Sample_Lib_C838_11	TGACTG	Sample2	3
Sample_Lib_C838_11	default	Sample3	4

The "Illumina sample" column corresponds to a directory with fastq files, as produced by bcl2fastq2.

Any observed barcode that does not match a barcode listed in the file (with a possible mismatch of 1) will be written to the sample specified by 'default'. Note that all barcodes must be of the same length. UMIs are also appended to read names. There can currently only be one UMI length per Illumina sample.
...
def readSampleTable(sampleTable):
    ...
    for line in f:
        ...
        if _[0] not in d:
            d[_[0]] = dict()
        UMIlengths[_[0]] = int(_[3])
        d[_[0]][_[1]] = _[2]
        if _[1] != 'default' and len(_[1]) > bcLen:
            bcLen = len(_[1])
    f.close()

    return (d, bcLen, UMIlengths)


def matchSample(sequence, sequence2, oDict, bcLen, buffer, UMIlen):
    """
    ...
    """
    bc = sequence[buffer + UMIlen:bcLen + buffer + UMIlen]
    bc2 = sequence2[buffer + UMIlen:bcLen + buffer + UMIlen] if sequence2 else None  # For whatever reason, padding isn't used
    UMI = None
    if UMIlen > 0:
        if sequence2:
            UMI = "{}{}".format(sequence[buffer:buffer + UMIlen], sequence2[buffer:buffer + UMIlen])
        else:
            UMI = "{}".format(sequence[buffer:buffer + UMIlen])
    ...
    return ("default", UMI, False)


def writeRead(lineList, of, bc, bcLen, args, UMIlen, UMI, doTrim=True):
    """
    ...
    if doTrim:
        ...
        lineList[1] = lineList[1][bcLen + args.buffer + UMIlen:]
        lineList[3] = lineList[3][bcLen + args.buffer + UMIlen:]

        # Fix the read name
        rname = rname.split()
        rname[0] = "{}:{}_{}".format(rname[0], bc, UMI)
        rname = " ".join(rname)
...
def writeRead2(lineList, of, bcLen, args, UMIlen, doTrim=True):
...
def processPaired(args, sDict, bcLen, read1, read2, UMIlen):
    ...
    for line1_1 in f1:
        ...
        (bc, UMI, isDefault) = matchSample(line1_2, line2_2, sDict, bcLen, args.buffer, UMIlen)

        rname = writeRead([line1_1, line1_2, line1_3, line1_4], sDict[bc][0], bc, bcLen, args, UMIlen, UMI, isDefault)
        writeRead2([rname , line2_2, line2_3, line2_4], sDict[bc][1], bcLen, args, UMIlen, isDefault)

    f1.close()
    f2.close()


def processSingle(args, sDict, bcLen, read1, UMIlen):
    f1_ = subprocess.Popen("gunzip -c {}".format(read1), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    f1 = f1_.stdout
    ...
    for line1_1 in f1:
        ...
        line1_3 = f1.next()
        line1_4 = f1.next()
        (bc, UMI, isDefault) = matchSample(line1_2, None, sDict, bcLen, args.buffer, UMIlen, isDefault)
        writeRead([line1_1, line1_2, line1_3, line1_4], sDict[bc], bc, bcLen, args, UMIlen, UMI, isDefault)

    f1.close()


def wrapper(foo):
    d, args, sDict, bcLen, UMIlen = foo
    ...
    if R2 is not None:
        ...
        if 'default' not in oDict:
            ...
            oDict[k] = [subprocess.Popen(['gzip', '-c'], stdout=open('{}/{}/{}_R1.fastq.gz'.format(args.output, d, v), "wb"), stdin=subprocess.PIPE).stdin,
                        subprocess.Popen(['gzip', '-c'], stdout=open('{}/{}/{}_R2.fastq.gz'.format(args.output, d, v), "wb"), stdin=subprocess.PIPE).stdin]
        processPaired(args, oDict, bcLen, R1, R2, UMIlen)
    else:
        ...
        if 'default' not in oDict:
            ...
            v = 'unknown'
            oDict[k] = [subprocess.Popen(['gzip', '-c'], stdout=open('{}/{}/{}_R1.fastq.gz'.format(args.output, d, v), "wb"), stdin=subprocess.PIPE).stdin]
        processSingle(args, oDict, bcLen, R1, UMIlen)


def main(args=None):
    args = parseArgs(args)

    sDict, bcLen, UMIlengths = readSampleTable(args.sampleTable)

    p = Pool(processes=args.numThreads)
    tasks = [(d, args, v, bcLen, UMIlengths[d]) for d, v in sDict.items()]
    p.map(wrapper, tasks)
